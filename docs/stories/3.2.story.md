...
1. [x] `list-products` command displays all products in a formatted table
2. [x] Table shows columns: ID, Name, Price (formatted with $)
3. [x] Products sorted by creation time (oldest first)
4. [x] Empty list displays helpful message ("No products stored yet.")
5. [x] `get-product "<name>"` command retrieves single product by name
6. [x] Get command displays: Name, Price, ID, Created At
7. [x] Product not found displays clear error message
8. [x] Both commands exit code 0 on success, 1 on not found/error
9. [x] Integration tests for both commands (8+ tests)
10. [x] All existing 193 tests still passing
11. [x] Zero RuboCop offenses maintained
12. [x] README.md updated with list/get examples
...
- [x] **Task 1: Implement list-products Command** (AC: 1, 2, 3, 4)
  - [x] Add `execute_list_products` method to CLI
  - [x] Check storage availability
  - [x] Call `Product.all` to retrieve all products
  - [x] Handle empty list with friendly message
  - [x] Format output as table (ID, Name, Price columns)
  - [x] Use `formatted_price` for price display
  - [x] Exit with code 0

- [x] **Task 2: Implement get-product Command** (AC: 5, 6, 7, 8)
  - [x] Add `execute_get_product` method to CLI
  - [x] Parse product name from command-line arguments
  - [x] Validate name argument (required, non-empty)
  - [x] Check storage availability
  - [x] Call `Product.find_by_name(name)` (case-insensitive)
  - [x] Handle product not found with error message
  - [x] Display product details (Name, Price, ID, Created)
  - [x] Exit code 0 on success, 1 on not found

- [x] **Task 3: Update CLI Routing** (AC: 1, 5)
  - [x] Add `list-products` case to command router
  - [x] Add `get-product` case to command router
  - [x] Update help text with new commands
  - [x] Add examples to help display

- [x] **Task 4: Write Integration Tests** (AC: 9)
...
- ✅ Story 3.1 complete (Product model with `all` and `find_by_name` methods)
- ✅ Database connection manager functional
- ✅ Test isolation pattern established (tmpdir)
- ✅ 185 tests passing, 0 RuboCop offenses
