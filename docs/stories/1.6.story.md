...
- [ ] **Task 1: Verify/Update Rakefile Install Task** (AC: 1, 2)
  - [ ] Open existing `Rakefile` (created in Story 1.1)
  - [ ] Verify `install` task exists
  - [ ] Update install task to copy `bin/pocket-knife` to `~/bin/`
  - [ ] Add `mkdir -p ~/bin` to create directory if needed
  - [ ] Add `chmod +x ~/bin/pocket-knife` to ensure executable permissions
  - [ ] Add success message: "Installed to ~/bin/pocket-knife"
  - [ ] Add PATH reminder: "Ensure ~/bin is in your PATH"
  - [ ] Test task with `rake install`
...
- [ ] **Task 5: Write Comprehensive README** (AC: 6, 7, 8)
...
- [ ] **Task 6: Add Uninstall Instructions** (AC: 8)
